...
public class AccountingReport implements Report {
    ...
    public String generate(Predicate<Employee> filter) {
        ...
        text.append("Name; Salary (").append(targetCurrency).append(");").append(System.lineSeparator());
        for (Employee employee : store.findBy(filter)) {
            double convertedSalary = converter.convert(Currency.RUB, employee.getSalary(), targetCurrency);
            text.append(employee.getName()).append(" ").append(convertedSalary).append(System.lineSeparator());
        }
